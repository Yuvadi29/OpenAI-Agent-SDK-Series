...
const getTemperatureTool = tool({
  ...
  //   This is where you need approval from user for the further action
  ...
  while (hasInterruptions) {
    ...
    for (const interruption of result.interruptions) {
      const confirmed = await confirm(
        `Agent ${interruption.agent.name} would like to use the tool ${
          interruption.name
        } with "${interruption.arguments || 'no arguments'}". Do you approve?`
      )
